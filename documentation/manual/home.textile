...
The @springtester@ module assumes you are wiring your application using the @spring@ module.  Here is an example of two classes being wired up using the @spring@ module.  The first is a @Person@ class that injects a @AgeCalculator@ and calls the @getAge()@ method:

bc. @Component
public class Person {
    @Resource private AgeCalculator calculator;
    public int getAge() {
        return calculator.calculate("1-January-1980");
    }
}

The @AgeCalculator@ returns 10:
...
bc. public final class PersonTest extends SpringMockitoUnitTestCase {
    ...
    @Mock AgeCalculator mockAgeCalculator;
    @Test
