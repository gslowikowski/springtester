...
The @springtester@ module is used in conjunction with the "spring":http://www.playframework.org/modules/spring module and "Mockito":http://code.google.com/p/mockito/ library and provides additional classes for injecting mocks/stubs when writing tests for your Play! application.
...
* The @PersonTest@ extends @SpringMockitoUnitTestCase@.
...
* The class @SpringMockitoUnitTestCase@ also provides the helper method @checkCount(Class<? extends Model> cls, long expected)@ which can be used to check the row count of table.
* The @SpringMockitoUnitTestCase@ will also delete the test in-memory database before each test method to ensure a clean run for each test method execution.
